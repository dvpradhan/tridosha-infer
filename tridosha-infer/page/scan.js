...
Page({
  ...
  build() {
    ...
    createWidget(widget.TEXT, { x: 0, y: 1220, w: 390, h: 40, color: 0xffffff, text_size: 28, align_h: align.CENTER_H, text: 'Current Symptoms' })
    createWidget(widget.FILL_RECT, { x: 145, y: 1265, w: 100, h: 3, color: 0x38bdf8, radius: 2 })
    createWidget(widget.FILL_RECT, { x: 15, y: 1290, w: 360, h: 340, color: 0x1e3a8a, radius: 12 }) 
    this.state.symptomWidget = createWidget(widget.TEXT, { x: 25, y: 1305, w: 340, h: 310, color: 0xbfdbfe, text_size: 20, text_style: text_style.WRAP, text: 'Symptom breakdown will appear after scan.' })

    createWidget(widget.TEXT, { x: 0, y: 1660, w: 390, h: 40, color: 0xffffff, text_size: 28, align_h: align.CENTER_H, text: 'Prescribed Routine' })
    createWidget(widget.FILL_RECT, { x: 15, y: 1720, w: 360, h: 320, color: 0x064e3b, radius: 12 }) 
    this.state.todoWidget = createWidget(widget.TEXT, { x: 25, y: 1735, w: 340, h: 290, color: 0xa7f3d0, text_size: 20, text_style: text_style.WRAP, text: '✅ TO-DO:\nWill appear after reading.' })
    
    createWidget(widget.FILL_RECT, { x: 15, y: 2060, w: 360, h: 320, color: 0x7f1d1d, radius: 12 }) 
    this.state.avoidWidget = createWidget(widget.TEXT, { x: 25, y: 2075, w: 340, h: 290, color: 0xfecaca, text_size: 20, text_style: text_style.WRAP, text: '❌ AVOID:\nWill appear after reading.' })

    createWidget(widget.FILL_RECT, { x: 0, y: 2420, w: 390, h: 20, color: 0x000000 })
  ...
  saveToHistory(analysis) {
    try {
      const now = new Date()
      const d = now.getDate(), m = now.getMonth() + 1, h = now.getHours(), min = now.getMinutes()
      const day = d < 10 ? '0' + d : d
      const month = m < 10 ? '0' + m : m
      const hours = h < 10 ? '0' + h : h
      const mins = min < 10 ? '0' + min : min
      
      // Extremely compact string to avoid any possible Zepp OS size limits
      const cleanTitle = analysis.title.replace('\n', ' ')
      const newEntry = `[ ${day}/${month} • ${hours}:${mins} ]\n◈ ${cleanTitle}\nPulse: ${Math.round(analysis.avgHR)} | HRV: ${analysis.hrv}`
      ...
      // Writing to brand new keys
      for (let i = 0; i < history.length; i++) {
        try { localStorage.setItem('td_log_' + i, String(history[i])) } catch(e) {}
      }

      showToast({ content: "Data Stored!" })
    } catch (e) {
      showToast({ content: "Storage Error." })
    }
  },
  ...
  calculateTridosha() {
    ...
    if (avgHR >= 70 && avgHR <= 75 && hrv >= 3 && hrv <= 4) {
      return { avgHR, hrv, title: "Sama-Dosha\n(Tridoshic)", detail: "State of 'Sama-Dosha'—a rare and perfect equilibrium. Air, Fire, and Earth are balanced.", symptoms: sympDict["Mixed"], todos: advice["Mixed"].t, avoids: advice["Mixed"].a }
    }
    ...
    if (base === sec) {
      ...
      return { avgHR, hrv, title: `Eka-Dosha:\nPure ${base}`, detail: details[base], symptoms: sympDict[base], todos: advice[base].t, avoids: advice[base].a }
    } else {
      const profiles = {
      ...
      return { avgHR, hrv, title: `Dvandvaja:\n${combo}`, detail: profiles[combo], symptoms: sympDict[dominant], todos: advice[adviceKey].t, avoids: advice[adviceKey].a }
    }
  },
